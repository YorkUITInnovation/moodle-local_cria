...
//    }
    ...
    if ($oldversion < 2025082400) {
        ...
        // Define field topic_options to be added to local_cria_bot.
        $table = new xmldb_table('local_cria_bot');
        $field = new xmldb_field('topic_options', XMLDB_TYPE_TEXT, null, null, null, null, null, 'timemodified');

        // Conditionally launch add field topic_options.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        // Define field topic_keywords to be added to local_cria_bot.
        $table = new xmldb_table('local_cria_bot');
        $field = new xmldb_field('topic_keywords', XMLDB_TYPE_TEXT, null, null, null, null, null, 'topic_options');

        // Conditionally launch add field topic_keywords.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
